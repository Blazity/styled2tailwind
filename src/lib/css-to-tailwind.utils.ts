...
>([
    ...
    (val) => {
      ...
      const res = vals.map((v) => {
        ...
        pipeV =
          pipeV.length > 0
            ? pipeV
            : v.replace(/^([a-zA-Z0-9_-]+)\((.+?)\)$/, (_r: string, k: string, v: any) => {
                canUsePipeV = true
                const result = backdropFilterValConfig[k]?.(v)
                if (result !== undefined) {
                  return result
                } else {
                  canUse = false
  ...
  ["border-image-width", (val) => (isUnit(val) ? `[border-image-width:${getCustomVal(val)}]` : "")],
  [
    "border-left",
    (val) => {
      if (isUnit(val)) {
      ...
      const res = vals.map((v) => {
        ...
        pipeV =
          pipeV.length > 0
            ...
            : v.replace(/^([a-zA-Z0-9_-]+)\((.+?)\)$/, (_r: string, k: string, v: any) => {
                canUsePipeV = true
                const result = filterValConfig[k]?.(v)
                if (result !== undefined) {
                  return result
                } else {
                  canUse = false
  ...
  ["flex-grow", (val) => (isUnit(val) ? { "0": "flex-grow-0", "1": "flex-grow" }[val] ?? `flex-grow-[${val}]` : "")],
  [
    "flex-shrink",
  ...
  [
    ...
    (val) =>
      isUnit(val)
        ? `h-${
            (useAllDefaultValues && getRemDefaultVal(val)) ||
            getUnitMetacharactersVal(val, [CustomSelect.vw]) ||
            `[${val}]`
          }`
        : "",
  ],
  ...
  [
    ...
    (val) => {
      ...
      return v === ""
        ...
        : v
            .split(" ")
            .map((t) => (t.includes("-") ? `-${t.replace("-", "").replace("_", "-")}` : t.replace("_", "-")))
            .join(" ")
    },
  ],
  [
    ...
    (val) => {
      ...
      return (
        ...
        (isUnit(val) ? `${t[0]}mb-${(useAllDefaultValues && getRemDefaultVal(t[1])) || `[${t[1]}]`}` : "")
      )
    },
  ],
  [
    ...
    (val) => {
      ...
      return (
        ...
        (isUnit(val) ? `${t[0]}ml-${(useAllDefaultValues && getRemDefaultVal(t[1])) || `[${t[1]}]`}` : "")
      )
    },
  ],
  [
    ...
    (val) => {
      ...
      return (
        ...
        (isUnit(val) ? `${t[0]}mr-${(useAllDefaultValues && getRemDefaultVal(t[1])) || `[${t[1]}]`}` : "")
      )
    },
  ],
  [
    ...
    (val) => {
      ...
      return (
        ...
        (isUnit(val) ? `${t[0]}mt-${(useAllDefaultValues && getRemDefaultVal(t[1])) || `[${t[1]}]`}` : "")
      )
    },
  ],
  ...
  [
    "max-height",
    (val) =>
      isUnit(val) ? { "0px": "max-h-0", "100%": "max-h-full", "100vh": "max-h-screen" }[val] ?? `max-h-[${val}]` : "",
  ],
  [
    "max-width",
    (val) =>
      isUnit(val)
        ? { none: "max-w-none", "100%": "max-w-full", "min-content": "max-w-min", "max-content": "max-w-max" }[val] ??
          `max-w-[${val}]`
        : "",
  ],
  [
    "min-height",
    (val) =>
      isUnit(val) ? { "0px": "min-h-0", "100%": "min-h-full", "100vh": "min-h-screen" }[val] ?? `min-h-[${val}]` : "",
  ],
  [
    "min-width",
    (val) =>
      isUnit(val)
        ? { "0px": "min-w-0", "100%": "min-w-full", "min-content": "min-w-min", "max-content": "min-w-max" }[val] ??
          `min-w-[${val}]`
        : "",
  ],
  [
      ...
      const transformValConfig: Record<string, (v: string) => string | undefined> = {
        scale: (v: string) => {
          ...
          if (vs[0] === vs[1] || vs.length === 1) {
            return `scale-${
              customTheme.scale?.[vs[0]] || (useAllDefaultValues && scaleDefaultVs[vs[0]]) || `[${vs[0]}]`
            }`
          }
          return vs
            .map((v, idx) => {
              return `scale-${idx === 0 ? "x" : "y"}-${
                customTheme.scale?.[v] || (useAllDefaultValues && scaleDefaultVs[v]) || `[${v}]`
              }`
            })
            .join(" ")
        },
        ...
        rotate: (v: string) => {
          ...
          if (vs.length > 1) {
            if (
              ...
              return `${t[0]}rotate-${
                customTheme.rotate?.[t[1]] || (useAllDefaultValues && rotateDefaultVs[t[1]]) || `[${t[1]}]`
              }`
            }
            return undefined
          }
          ...
          return vs
            .map((v, idx) => {
              ...
              return `${t[0]}translate-${idx === 0 ? "x" : "y"}-${
                customTheme.translate?.[t[1]] || (useAllDefaultValues && translateDefaultVs[t[1]]) || `[${t[1]}]`
              }`
            })
            .join(" ")
        },
          ...
          return vs
            .map((v, idx) => {
              ...
              return `${t[0]}skew-${idx === 0 ? "x" : "y"}-${
                customTheme.skew?.[t[1]] || (useAllDefaultValues && skewDefaultVs[t[1]]) || `[${t[1]}]`
              }`
            })
            .join(" ")
        },
...
export const parsingCode = (code: string): CssCodeParse[] => {
  code = code.replace(/[\n\r]/g, "").trim()
...
export const getResultCode = (it: CssCodeParse, prefix = "", config: TranslatorConfig) => {
  ...
  return {
    ...
    resultVal: [...new Set(formattedResultVals)].join(" "),
  }
}

export const getPropertyPipeValue = (
  key: string,
  value: string,
...
export const getFallbackPropertyValue = (key: string, value: string): string => {
  if (!value || !key) {
    return undefined
  }
...
export const applyPrefixesAndImportant = (pipeVal: string, isNegativeProperty: boolean, prefix: string): string => {
  if (!pipeVal) {
    return ""
  }
